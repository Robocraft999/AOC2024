...
for m in re.findall(r"Button A: X\+(\d+), Y\+(\d+)\nButton B: X\+(\d+), Y\+(\d+)\nPrize: X=(\d+), Y=(\d+)", text):
    ...
    lb = np.array([int(m[4]) + 10000000000000, int(m[5]) + 10000000000000])
...
print(sum([sum([np.sum((3, 1) * np.rint(x))for x,padd in [(np.linalg.solve(np.array([[int(m[0]), int(m[2])], [int(m[1]), int(m[3])]]), np.array([int(m[4]) + 10000000000000, int(m[5]) + 10000000000000])), 1e-4)]if abs(np.rint(x)[0] - x[0]) < padd and abs(np.rint(x)[1] - x[1]) < padd]) for m in re.findall(r"Button A: X\+(\d+), Y\+(\d+)\nButton B: X\+(\d+), Y\+(\d+)\nPrize: X=(\d+), Y=(\d+)", functools.reduce(lambda x, y: x+y,[line for line in open(get_input(13)[0])]))]))
